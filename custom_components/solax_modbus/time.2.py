...
class SolaXModbusTime(TimeEntity):
    ...
    def native_value(self) -> float:
        ...
        _LOGGER.info(f"*** Read entity_description {descr}")
        if descr.prevent_update:
            if  (self._hub.tmpdata_expiry.get(descr.key, 0) > time()):
                ...
                if val == None:
                    ...
                    val = 0
                if (descr.read_scale and self._hub.tmpdata[self._key]): res = val*descr.read_scale
                else: res = val
        ...
        if self._key in self._hub.data:
            try:    val = self._hub.data[self._key]*descr.read_scale
            except: val = self._hub.data[self._key]
            return val
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        if self._write_method == WRITE_MULTISINGLE_MODBUS:
            _LOGGER.info(f"writing {self._platform_name} {self._key} time register {self._register} value {payload} after div by readscale {self.entity_description.read_scale} scale {self._attr_scale}")
